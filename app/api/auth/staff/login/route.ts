...
// is RLS-on / no-policy). On success it returns the staff session object —
// the browser stores it in localStorage (lib/simpleStaffSession.ts). No
// SESSION_SECRET and no signed cookie are involved.
...
export async function POST(req: Request) {
  ...
  const res = await admin
    .from("staff_accounts")
    .select("id, employee_code, password_hash, full_name, role, branch_id, active")
  ...
  // The session object is stored by the browser in localStorage.
  return NextResponse.json({
    ok: true,
    session: {
      staffId: staff.id,
